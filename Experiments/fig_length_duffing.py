...
for p in [0,1]:	
	for lam in list(10.**(np.arange(-10,1,1))):
...
def fig_length_duffing(Ntrial = 10, N = 220, points = 9, dt = 1e-2, noise = 1e-3, lengths = None):
	phis, C = lide.duffing()
	x0 = np.array([1,0])
	t = dt * np.arange(N)
	...
	for i, length in enumerate(lengths):
		for j, seed in enumerate(range(Ntrial)):
			...
			for k, alg in enumerate(algs):
				...
				pgf.write(f'data/fig_length_duffing_{names[k]}.dat')
...
if __name__ == '__main__':
	...
	fig_length_duffing(Ntrial = 20, N = int(100/dt), dt = dt, points = 9, lengths = lengths)
