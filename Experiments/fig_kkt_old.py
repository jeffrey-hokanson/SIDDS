...
def build_kkt(Ys, phis, C_old, Xs_old, dts, points = 3, weight = None, x0 = None,
	...
	b3 = KKT_LHS.shape[0]
	if rho > 0:
		ilu3 = scipy.sparse.linalg.splu(( -rho*scipy.sparse.eye(B.shape[0]) + (B @ B.T)).tocsc())
		M3 = scipy.sparse.linalg.LinearOperator((b3 - b2, b3 - b2), lambda x: ilu3.solve(x))
	else:
		M3 = scipy.sparse.linalg.LinearOperator((b3 - b2, b3 - b2), lambda x: x)
...
if __name__ == '__main__':
	...
	A, b, M, x0 = build_kkt(Ys, phis, C_old, Ys, dts, points = 9)

	# without preconditioning
	class OperatorWrapper(scipy.sparse.linalg.LinearOperator):
		def __init__(self, A):
			self.A = A
			self.xs = []
		@property
	...
	AA = OperatorWrapper(A)
	#gmres(A, b,x0 = x0, tol = 1e-10, maxiter = 1000, residuals = res)
	scipy.sparse.linalg.gmres(AA, b, M = M, x0=x0, tol = 1e-20,  restart = 1000, maxiter = 1)
	res = [np.linalg.norm(A @ x - b) for x in AA.xs]
	pgf = PGF()
	...
	res = []
	AA = OperatorWrapper(A)
	scipy.sparse.linalg.gmres(AA, b, x0=x0, tol = 1e-20,  restart = 1000, maxiter = 1)
	res = [np.linalg.norm(A @ x - b) for x in AA.xs]
	pgf = PGF()
	pgf.add('residual', res)
	pgf.add('iter', np.arange(len(res)))
